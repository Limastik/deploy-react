...
const Header = () => {
  ...
  const filmsList = ["Stranger Things", "Breaking Bad", "The Rain", "Life In a Year", "Squid Game"];
  let [resultName, setResultName] = useState("No Movie Found");

  let [searchBoxStyle, setSearchBoxStyle] = useState("serch-box hidden");

  const [showElement, setShowElement] = useState(true);
  useEffect(() => {
    if (!showElement) {
      setSearchBoxStyle("serch-box")
    }
    else{
      setSearchBoxStyle("serch-box hidden")
    }
  }, [showElement]);
  const handleClick = () => {
  ...
  function UpdateFilmName(e){
    const name = e.target.value;
    if (name.length > 3){
      let targetName = filmsList.filter(item => item.startsWith(name));

      if (targetName != undefined)
        setResultName(targetName);
      else
        setResultName("No Movie Found");
    }
    else{
      setResultName("No Movie Found");
    }
  ...
  return (
    <header className="header">
      ...
      <div className={searchBoxStyle}>
        <input 
        type="text"
        onChange={(e) => UpdateFilmName(e)}></input>
